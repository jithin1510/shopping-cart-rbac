import {Box, FormHelperText, Stack, TextField, Typography, useMediaQuery, useTheme, Alert } from '@mui/material'
...
export const Login = () => {
  ...
  useEffect(() => {
    if(status === 'fullfilled' && loggedInUser?.isVerified === true) {
      ...
      if (loggedInUser.role === 'vendor' && !loggedInUser.isApproved) {
        message = 'Login successful. Your vendor account is pending approval.';
      }
  ...
  }, [status, dispatch, reset, loggedInUser])
