...
exports.signup=async(req,res)=>{
    try {
        ...
        if (req.body.role && ['customer', 'vendor', 'admin'].includes(req.body.role)) {
            // If role is vendor, set isApproved to false (needs admin approval)
            if (req.body.role === 'vendor') {
                req.body.isApproved = false;
            }
            // If role is customer, isApproved is true by default
            // If role is admin, this should be set manually by a super admin (not through signup)
            if (req.body.role === 'admin') {
                req.body.role = 'customer'; // Default to customer if trying to register as admin
            }
        } else {
...
exports.resendOtp=async(req,res)=>{
    try {
        ...
        await sendMail(existingUser.email,`OTP Verification for Your Shopping Cart Account`,`Your One-Time Password (OTP) for account verification is: <b>${otp}</b>.</br>Do not share this OTP with anyone for security reasons`)
...
exports.forgotPassword=async(req,res)=>{
    ...
    try {
        ...
        await sendMail(isExistingUser.email,'Password Reset Link for Your Shopping Cart Account',`<p>Dear ${isExistingUser.name},

        We received a request to reset the password for your Shopping Cart account. If you initiated this request, please use the following link to reset your password:</p>
...
exports.logout=async(req,res)=>{
    try {
        ...
        res.cookie('token', '', {
            ...
            sameSite: process.env.PRODUCTION === 'true' ? 'None' : 'Lax',
            maxAge: 0  // This will set Max-Age=0 in the cookie
        });
